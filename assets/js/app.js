...
function ToolTipUpdate(XAxis, YAxis, GroupCircles) {
    ...
    var toolTip = d3.tip()
        ...
        .offset([-8, 0])
        .html(function(d) {
            return (`${d.state}<br>${xLabel} ${Xstyle(d[XAxis], XAxis)}<br>${yLabel} ${d[YAxis]}%`);
        });
...
d3.csv("./assets/data/data.csv").then(function(censusData) {
    ...
    var LinearScaleX = xScale(censusData, XAxis);
    var LinearScaleY = yScale(censusData, YAxis);
    ...
    xLabelsGroup.selectAll("text")
        .on("click", function() {
            ...
            if (value != XAxis) {
                ...
                LinearScaleX = xScale(censusData, XAxis);
